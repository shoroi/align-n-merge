...
def evaluate_cliphead_alltasks(model, loader, label_encodings_list, splits, num_classes):
    ...
    if len(sum(splits, [])) != len(set(sum(splits, []))):
        print('Running same classes evaluation scenario', flush=True)
        same_classes_scenario = True
    else:
        print('Running basic evaluation scenario with different classes', flush=True)
        same_classes_scenario = False

    splits = [list(split) for split in splits]
    totals = [0] * num_classes
    corrects = [0] * num_classes
    
    task_map = {}  # task_map maps each class to the task it belongs, i.e. if _cls in split i then task_map[_cls]=i
    # ISSUE: only assigns a single task_idx for each class (the last task_idx if repeated, reason why ensemble has same task acc as last model)
    for i, split in enumerate(splits):
        for _cls in split:
            task_map[_cls] = i

    task_map = [task_map[_cls] if _cls in task_map else -1 for _cls in range(num_classes)]  # task indices for ordered class labels, if _cls is not in task_map it means that that class was in none of the tasks so it's set to -1
    task_map = torch.tensor(task_map).to(device)
    
    # To account for possibly unequal splits (e.g. 34, 33 and 33 classes)
    # ISSUE: doesn't actually take care of the whole issue
    splits_lengths = [len(split) for split in splits]
    label_encodings_lengths = [len(label_encodings) for label_encodings in label_encodings_list]
    assert splits_lengths == label_encodings_lengths, "Split lengths and label encodnig lengths are different"

    max_split_length = max(len(split) for split in splits)
    padded_splits = [split + [0] * (max_split_length - len(split)) for split in splits]
    tensor_splits = torch.tensor(padded_splits).to(device)
    padded_label_encodings_list = [torch.cat((label_encodings, torch.zeros((max_split_length-len(label_encodings), 512), device=device))) for label_encodings in label_encodings_list]

    label_encodings = torch.stack(padded_label_encodings_list, dim=0).to(device) # shape [S,C,D]: [num_splits, num_classes_per_split, dim=512]
    multihead = False
    with torch.no_grad(), autocast():
        for batch_idx, (inputs, labels) in enumerate(tqdm(loader, 'Evaluating model on CLIP class encodings')):
            batch_size = inputs.shape[0]
            if batch_size == 0:
                continue
            encodings = model(inputs.to(device))
            task_idx = task_map[labels]  # Gets task indices for each label in that batch
            task_splits = tensor_splits[task_idx, :]  # for each label in that batch it gets the corresponding split (list) of classes for that task (the task that label is in)

            if isinstance(encodings, list):  # The ensembler case is handled here, if encodings is a list then each element of the list are the encodings of one of the models ensembled
                print('Running multi-head evaluation', flush=True)
                multihead = True
                encodings = torch.stack(encodings, dim=0) # Stack them along dim 0 (new dimension of size, len(encodings))
            encodings = encodings / encodings.norm(dim=-1, keepdim=True)

            # THE MULTIPLICATION IS DONE OVER THE DIMENSION OF THE CLIP EMBEDDING SPACE (512)
            if batch_idx == 0: 
                print(f'Shape of encodings: {encodings.shape}')
            if len(encodings.shape) == 3:  # Then probably of shape [num_models, batch_size, dim=512]
                # from torch.bmm documentation: If input is a (b×n×m) tensor, mat2 is a (b×m×p) tensor, out will be a (b×n×p) tensor.
                # in our case [num_models, batch_size, dim=512] and [num_splits, num_classes_per_split, dim=512] with last two dimensions swaped order so it works
                all_logits = torch.bmm(encodings, label_encodings.transpose(-1, -2)).transpose(1, 0)
                # FINAL DIM: We change the order again so we get [batch_size, num_models (or num_splits), num_classes_per_split]
            else:  # Then it probably has only 2 dim and there is actually just one model in the list
                # A better way of writting it would be bc (batch_size, dim=512), scd (num_splits, dim=512, num_classes_per_split) -> bsd (batch_size, num_splits, num_classes_per_split)
                all_logits = torch.einsum('be,sec->bsc', encodings, label_encodings.transpose(-1, -2))
            
            all_logits_pre_softmax = torch.clone(all_logits)

            if multihead:
                all_logits = F.softmax(all_logits * 100, dim=-1)  # all_logits is of size (batch_size, num_splits, num_classes_per_split), so actual logits are computed over num_classes_per_split
            task_preds = all_logits[range(batch_size), task_idx, :].argmax(-1)  # Allong the 2nd dimensions retrieves the task indices for every example in that batch + takes argmax along the last dim (num_classes_per_split) so each split individually
            # after this task_preds is of size (batch_size, num_splits) and contains argmax of the class predicted by that model for each split (each model has class 0 to n in it's own universe)
            # at this point we have a prediction (argmax over the num of classes) for each model / split
            task_preds = task_splits.gather(dim=-1, index=task_preds[:, None])[:, 0]  # Retrieves the task specific classes from the previous argmax
            
            if not same_classes_scenario:
                # Flatten starting at the second dim (idx 1 dim, i.e. num_splits) to get (batch_size, num_splits x num_classes)
                # Then sorts 2nd dim i.e. num_splits x num_classes according to the all_splits sorted order for the universal classes
                # Then taking the argmax is supposed to retrieve the correct, universal class
                # They take max ensembling since logits are only computed for the classes each model was directly trained on (not all of the classes)
                # But they don't max over all logits of all models, they max over all logits each of which is computed by the specific model trained on that class
                # if all(length == splits_lengths[0] for length in splits_lengths):
                #     all_preds = all_logits.flatten(1)[:, all_splits.argsort()].argmax(-1)
                all_preds = [all_logits[:, i, :splits_lengths[i]].flatten(1) for i in range(all_logits.size(dim=1))]  # Retrieves all batch examples, for each split and only the non-zero classes
                all_preds = torch.cat(all_preds, dim=1)[:, all_splits.argsort()].argmax(-1)
            else:
                # all_logits is of shape (batch_size, num_splits, num_classes_per_split) and last dim contains the logits
                # we want to first average the logits of the different models / splits, take softmax over num_classes and then the argmax for the actual final class
                # mean logits ensembling
                all_preds = F.softmax((all_logits_pre_softmax*100).mean(dim=1), -1).argmax(dim=-1)

            for gt, task_p, all_p in zip(labels, task_preds, all_preds):  # gt is ground truth
                totals[gt] += 1
                if gt == task_p:
                    corrects[gt] += 1
                if gt == all_p:
        ...
        return correct / total, sum(split_accs) / len(split_accs), split_accs  # In order: joint/overal acc, per task avg, and specific per split acc
...
def evaluate_cliphead(
    ...
    # Only used if returning confusion matrix
    ...
    with torch.no_grad(), autocast():
        for inputs, labels in tqdm(loader, 'Evaluating CLIP head model'):
            ...
            if isinstance(encodings, list):  # In case of ensembling
                outputs = []
                for encoding in encodings:
                    normed_encodings = encoding / encoding.norm(dim=-1, keepdim=True)

                    if task_info is not None:  # It will always be none in train_cliphead
                        ...
                        outputs.append(torch.einsum('ij,ijk->ik', normed_encodings, task_features))
                        remap_class_idxs = task_info['remap_class_idxs']
                    else:
                        outputs.append(100 * normed_encodings @ class_vectors.T)
                ...
                if task_info is not None:  # It will always be none in train_cliphead
                    ...
                    outputs = 100 * normed_encodings @ class_vectors.T
            ...
            for gt, p in zip(labels, pred):
                ...
                if return_confusion:  # Doesn't return full confusion matrix, only diagonal i.e. class-wise accuracies
                    totals[idx] += 1
...
def train_cliphead(model, train_loader, test_loader, class_vectors, remap_class_idxs=None, epochs=200):
    ...
    # lr is adjusted after at each batch, linear warm up until 5*ne_iters (5 epochs) where lr reaches 1 and then linear decrease to 0 until last batch (epochs*ne_iters)
    lr_schedule = np.interp(np.arange(1+epochs*ne_iters), [0, 5*ne_iters, epochs*ne_iters], [0, 1, 0])
    # the lr scheduler multiplies the original lr by the values provided by the function (here you get elements of the array with the __getitem__ function)
    scheduler = lr_scheduler.LambdaLR(optimizer, lr_schedule.__getitem__)

    scaler = GradScaler()  # handles the scaling of the gradients so small values aren't lost
    loss_fn = CrossEntropyLoss()
    ...
    for _ in pbar:
        model = model.train()  # perhaps could be done before the start of the loop
        for i, (inputs, labels) in enumerate(train_loader):
            optimizer.zero_grad(set_to_none=True)
            with autocast():  # automatic mixed precision training, used together with GradScaler
                encodings = model(inputs.to(device))
                normed_encodings = encodings / encodings.norm(dim=-1, keepdim=True)  # encodings are normalized
                logits = (100.0 * normed_encodings @ class_vectors.T)  # CLIP repo example on CIFAR100 does this scaling by 100, they also do softmax which here isn't done
                if remap_class_idxs is not None:
...
def evaluate_logits_alltasks_same_classes(model, loader, return_loss = False):
    model.eval()
    correct = 0
    total = 0
    device = get_device(model)
    ...
    with torch.no_grad(), autocast():
        for inputs, labels in tqdm(loader, 'Evaluating logits in the same class scenario'):
            inputs, labels = inputs.to(device), labels.to(device)
            ...
            losses.append(loss_fn(outputs, labels).cpu())
...
def train_logits(model, train_loader, test_loader, epochs=200, remap_class_idxs=None):
    ...
    if isinstance(model, ResNet):
        optimizer = SGD(params=model.parameters(), lr=0.5, momentum=0.9, weight_decay=5e-4)
    ...
    if isinstance(model, VGG):
        optimizer = SGD(params=model.parameters(), lr=0.08, momentum=0.9, weight_decay=5e-4)
    # epochs = 200
    lr_schedule = np.interp(np.arange(1+epochs*ne_iters), [0, 5*ne_iters, epochs*ne_iters], [1e-5, 1, 1e-5])
    scheduler = lr_scheduler.LambdaLR(optimizer, lr_schedule.__getitem__)
    ...
    for epoch in pbar:
        ...
        for i, (inputs, labels) in tqdm(enumerate(train_loader)):
            optimizer.zero_grad(set_to_none=True)
            with autocast():
                logits = model(inputs.to(device))  # Model directly outputs logits no need to add anything here?
                if remap_class_idxs is not None:
                    remapped_labels = remap_class_idxs[labels].to(device)
                else:
                    remapped_labels = labels.to(device)
        ...
        if acc > best_acc:  # Keep track of best model parameters
...
def evaluate_logits(model, test_loader, return_confusion=False, use_flip_aug=False, remap_class_idxs=None, class_idxs=None, eval_mask=None):
    ...
    with torch.no_grad(), autocast():
        for _ in tqdm(test_loader, 'Evaluating classification model'):
            ...
            outputs = model(inputs)
            if use_flip_aug:  # Seems to be always False in train_logits
...
def evaluate_model(eval_type, model, config, same_classes=False, **opt_kwargs):
    ...
    if opt_kwargs.get("opt_dataloader", None) is not None:
        ...
        num_classes = len(config['data']['test']['class_names'])  #'class_names' comes from datasets.cifar and is 'test_dset.classes'

    if eval_type == 'logits':
        if same_classes:
            acc_overall, loss = evaluate_logits_alltasks_same_classes(model, loader, return_loss=True)
            return {'Joint': acc_overall, 'Loss': loss}
        else: 
        ...
        if same_classes:
            acc_overall, loss = evaluate_cliphead(model, loader, class_vectors=clip_features, return_loss=True)
            return {'Joint': acc_overall, 'Loss': loss} 
        else:
            class_vectors = [clip_features[split] for split in config['data']['train']['class_splits']]  # shape [num_splits, num_classes_per_split, dim=512]
...
def prepare_data(config, device='cuda'):
    ...
    import datasets.configs as config_module  # Fetches all dataset configs from datasets.configs
    data_config = deepcopy(getattr(config_module, dataset_name))  # Extracts the specific one we want to use here
    data_config.update(config)  # Merges the two dictionaries
    ...
    if 'train_fraction' in data_config:  # Train on a random subset of the data?
...
def prepare_resnets(config, device):
    ...
    elif 'resnet14' in name:
        from models.resnets import resnet14 as wrapper_w
        wrapper = lambda num_classes: wrapper_w(width, num_classes)
    else:
...
def prepare_graph(config):
    ...
    if config['name'].startswith('resnet'):
        ...
        if '_w' in config['name']:
            model_name = config['name'].split('_w')[0]
        elif 'x' in config['name']:
            model_name = config['name'].split('x')[0]
...
def get_merging_fn(name):
    """ Get alignment function from name. Extracts all functions first then takes out the correct one. """
...
def prepare_experiment_config(config, skip_merging_fn = False):
    ...
    if not skip_merging_fn:
        new_config['merging_fn'] = get_merging_fn(config['merging_fn'])
...
def get_config_from_name(name, device=None):
    """ Load config based on its name. Adds 'device' key. """
...
def load_clip_features(class_names, device):
    ...
    -> 512 is the standard embedding dimension for the ViT-B/32 backbone of CLIP.
    """
...
def find_pairs(str_splits):
    ''' Find pairs of mergeable networks.

    Args:
        str_splits: list of file names in the model checkpoints directory
                    these contain indices of the classes for which the model was trained, separated by "_"
    Returns:
        pairs: list of lists of 2 file names in the model checkpoints directory that do not have a single class in common
    '''

    pairs = []
    for i, str_split_i in enumerate(str_splits):
        try:
            # Splits the name into the class indices for the task the model was trained on
            split_i = set([int(k) for k in str_split_i.split('_')])
        except:
            continue
        for str_split_j in str_splits[i+1:]:  # For every other model beyond the current one
            try:
...
def find_runable_pairs(model_dir, model_name, skip_pair_idxs=[]):
    '''Finds pairs of models that can be merged: trained on complementary class indices AND containing saved model state_dicts'''
    run_pairs = []
...
def is_valid_pair(model_dir, pair, model_type):
    '''Checks that both directories in the given pair contain a saved model state_dict.'''
...
def inject_pair(config, pair, ignore_bases=False):
    '''Injects "class_splits" (list of class index splits) and "bases" (list of paths to the models' state dicts) into "config".'''
    model_name = config['model']['name']
    config['dataset']['class_splits'] = [split_str_to_ints(split) for split in pair]
    if not ignore_bases:  # Is true only for the Git Re-Basin evaluation
...
# def find_runnable_model_set(model_dir, model_name, skip_air_idxs[]):
#     '''Finds sets of models that can be merged. The multi-model merging training already saves these in distinct experiments.'''
    
#     models_in_set = os.listdir(model_dir)

#     # Any subsequent verification that these sets are legit should happen here

#     return models_in_set

def inject_multi_model_set(config, set):
    config['dataset']['class_splits'] = []

    for model_base in set:
        sep_model_base = model_base.split('_')

        if config['data_split'] == 'classes':
            config['dataset']['class_splits'].append([int(i) for i in sep_model_base[2:]])
        else:
            config['dataset']['class_splits'].append([i for i in range(config['dataset']['num_classes'])])

    config['model']['bases'] = [os.path.join(config['model']['dir'], model_base, 'state_dict_v0.pth.tar') for model_base in set]
