...
from core.metric_calculators import CovarianceMetric, MeanMetric
from core.matching_functions import match_tensors_zipit, match_tensors_identity
from core.utils import get_merging_fn


class MergeHandler:
    """ 
    Handles all (un)merge transformations on top of a (single) graph architecture. merge/unmerge is a dict whose 
    ...
    def handle_conv2d(self, forward, node, module):
        ...
        if forward: # unmerge - only true in forward prop
            try:
                if module.weight.shape[1] == self.unmerge.shape[0]:
                    module.weight.data = torch.einsum('OIHW,IU->OUHW', module.weight, self.unmerge)
                else:
                    unmerge = torch.eye(module.weight.shape[1], device=self.unmerge.device)
                    module.weight.data = torch.einsum('OIHW,IU->OUHW', module.weight, unmerge)
            except:
                pdb.set_trace()
        else: # merge
            try:
                if module.weight.shape[0] == self.merge.shape[1]:
                    merge = self.merge
                    module.weight.data = torch.einsum('UO,OIHW->UIHW', merge, module.weight)
                else:
                    merge = torch.eye(module.weight.shape[0], device=self.unmerge.device)
                    module.weight.data = torch.einsum('UO,OIHW->UIHW', merge, module.weight)
            except:
                pdb.set_trace()
            if hasattr(module, 'bias') and module.bias is not None:
                module.bias.data = merge @ module.bias
    ...
    def handle_linear(self, forward, node, module):
        ...
        if forward: # unmerge
            if module.weight.shape[1] == self.unmerge.shape[0]:
                ...
                unmerge = torch.eye(module.weight.shape[1], device=self.unmerge.device)
                module.weight.data = module.weight @ unmerge
        else:
...
class ModelMerge(nn.Module):
    ...
    def compute_metrics(self, dataloader, metric_classes):
        ...
        for dataloader in dataloader_list:
            for x, _ in tqdm(dataloader, desc="Forward Pass to Compute Merge Metrics: "):
                ...
                numel += batch_size
                # List of dicts each dict containing intermediates for one network/graph
                intermediates = [g.compute_intermediates(x) for g in self.graphs]
                nodes = list(intermediates[0].keys())  # extracts node names from first graph
                if self.metrics is None:
                    # Define a metrics dict, each node key contains the different metrics values
                    self.metrics = {node: {key: value() for key, value in metric_classes.items()} for node in nodes}
                
                for node, node_metrics in self.metrics.items():
                    for metric in node_metrics.values():
                        # Extracts the intermediates at the same node for the different graphs
                        # and updates the metrics accordingly
                        intermeds_float = [i[node].float() for i in intermediates]
                        metric.update(batch_size, *intermeds_float)
        
        for node, node_metrics in self.metrics.items():
            for metric_name, metric in node_metrics.items():
                self.metrics[node][metric_name] = metric.finalize(numel)

        print(f'Computing metrics completed in: {time() - start_time}', flush=True)
        return self.metrics
    ...
    def compute_transformations(self, transform_fn, reduce_ratio=.5, **kwargs):
        ...
        nodes.sort()  # Somehow the nodes are ints (index of node) and not just name
        print(f'start_at: {self.start_at} and stop_at: {self.stop_at}', flush=True)

        for node in tqdm(nodes, desc="Computing transformations: "):
            if self.start_at is None or node >= self.start_at:
                print(f'computing transforms for node {node}', flush=True)
                ...
                # .chunk attempts to split tensor into specified number of chunks (num graphs here) along the specified dimension
                # takes care of the fact that merge = unmerge.T so not same dimensions
                self.merges[node] = merge.chunk(len(self.graphs), dim=1)
                self.unmerges[node] = unmerge.chunk(len(self.graphs), dim=0)

                if self.save_transforms and self.save_dir and (transform_fn != match_tensors_identity):
                    save_dir = f'{self.save_dir}/{transform_fn.__name__.split("_")[-1]}_transforms'
                    os.makedirs(save_dir, exist_ok=True)
                    torch.save(merge, f'{save_dir}/merge_node{node}.pt')
                    torch.save(unmerge, f'{save_dir}/unmerge_node{node}.pt')
                if self.save_covariances and self.save_dir:
                    save_dir = f'{self.save_dir}/metrics'
                    os.makedirs(save_dir, exist_ok=True)
                    torch.save(metric["covariance"], f'{save_dir}/covariances{node}.pt')
                
                if self.stop_at is not None and node == self.stop_at:
                    break
        
        self.compute_transform_time = time() - start_time
        print(f'Computing transforms completed in: {self.compute_transform_time}', flush=True)
        return self.merges, self.unmerges
    
    def apply_transformations(self):
        """
        Applies transformations found by compute_transformations from start_at up to stop_at graph node location 
        on all graph models. 
        """
        for node in self.merges:
            # Gets all merge and unmerge matrices for every graph, for a specific node
            merges = self.merges[node]
            unmerges = self.unmerges[node]
            for merge, unmerge, graph in zip(merges, unmerges, self.graphs):

                print(f'Applying transformation with merging style: {self.merging_style}')
                if self.merging_style == "both":
                    merger = MergeHandler(graph, merge, unmerge)
                elif self.merging_style == "merge_only":
                    id_unmerge = torch.eye(unmerge.shape[0]).to(self.device)
                    merger = MergeHandler(graph, merge, id_unmerge)
                elif self.merging_style == "unmerge_only":
                    id_merge = torch.eye(merge.shape[0]).to(self.device)
                    merger = MergeHandler(graph, id_merge, unmerge)
    ...
    def get_merged_state_dict(self, interp_w=None):
        ...
        keys = list(self.graphs[0].model.state_dict().keys())  # Retrieves parameter names from the first graph in list
        try:
            for key in keys:
    ...
    def transform(self, model,
        ...
        self.save_covariances = save_covariances
        self.save_dir = save_dir
        self.merged_model = model.to(self.device)
        self.merging_style = merging_style
