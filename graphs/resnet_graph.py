...
class ResNetGraph(BIGGraph):
    ...
    def add_basic_block_nodes(self, name_prefix, input_node):
        ...
        if shortcut_prefix in self.named_modules and len(self.get_module(shortcut_prefix)) > 0:
            ...
            shortcut_output_node = self.add_nodes_from_sequence(
                ...
                list_of_names=['0', '1'],
                # list_of_names=['0', NodeType.PREFIX, '1'],
                input_node=input_node
            )
        ...
        skip_node = self.add_nodes_from_sequence(
            ...
            list_of_names=[
                ...
                # 'conv1', NodeType.PREFIX, 'bn1', 'conv2', 'bn2', NodeType.SUM],
            input_node=input_node
        )
    ...
    def add_layer_nodes(self, name_prefix, input_node):
        # name_prefix just adds a prefix to the node's name
    ...
    def graphify(self):
        ...
        input_node = self.add_nodes_from_sequence('', ['conv1', 'bn1'], input_node, sep='')  # Returns last node added
        # input_node = self.add_nodes_from_sequence('', ['conv1', NodeType.PREFIX, 'bn1'], input_node, sep='')  # Returns last node added
        ...
        input_node = self.add_nodes_from_sequence('', 
            ...
            # ['avgpool', self.head_name, NodeType.OUTPUT], input_node, sep='')
...
if __name__ == '__main__':
    ...
    graph1.draw(nodes=range(len(graph1.G)), save_path='./figures/resnet_graph_all.png')
    # graph1.draw(nodes=range(20), save_path='./figures/resnet_graph_0-20.png')
    # graph1.draw(nodes=range(len(graph1.G)-20, len(graph1.G)), save_path='./figures/resnet_graph_20-end.png')
