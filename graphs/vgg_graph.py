...
class VGGGraph(BIGGraph):
    ...
    def __init__(self, model, architecture, pre_activ=False):
        super().__init__(model)
        self.architecture = architecture
        self.pre_activ = pre_activ
    ...
    def graphify(self):
        ...
        for arch_idx, elem in enumerate(self.architecture):
            if elem == 'M':
                ...
                if self.pre_activ: graph.append(node_insert)
                graph.append('features.' + str(graph_idx + 1)) # ReLU
                if not self.pre_activ: graph.append(node_insert)
                graph_idx += 2
        # pdb.set_trace()
...
def vgg11(model, pre_activ=False):
    architecture = [64, 'M', 128, 'M', 256, 256, 'M', 512, 512, 'M', 512, 512, 'M']
    return VGGGraph(model, architecture, pre_activ=pre_activ)

def vgg16(model, pre_activ=False):
...
if __name__ == '__main__':
    ...
    graph1 = vgg11(deepcopy(model), True).graphify()
    graph2 = vgg11(deepcopy(model), True).graphify()
    ...
    graph1.draw(
        save_path='./graphs/vgg11_graph_auto.png'
    )
