...
class BIGGraph(ABC):
    ...
    def reset_graph(self):
        """ Create New Graph. """
        self.G = nx.DiGraph()  # Directed graph with self loops
    ...
    def add_hooks(self, device=0):
        ...
        for node in self.G:
            ...
            if info['type'] == NodeType.PREFIX:
                ...
                for succ_node in self.G.succ[node]:  # Iterate through successors if PREFIX
                    succ_info = self.get_node_info(succ_node)
                    if succ_info['type'] == NodeType.MODULE:  # If successor is a pytorch module
    ...
    def compute_intermediates(self, x):
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            self.intermediates = {}
            self.model(x)
            # The hooks placed by add_hooks() are automatically called before .forward()
            # and the intermediates are directly saved in each hook in self.intermediates
